...
class Subgraph(ApiSpecs):
    ...
    def process_response(self, response: HttpMessage) -> Any:
        ...
        if res is None and self.response_info.error_data is None:
            fallback: Any
            expected_type = self.response_info.response_type
            ...
            self.context.logger.debug(
                "Response missing expected data for %s. Returning empty %s.",
                self.api_id,
                expected_type,
            )
            return fallback

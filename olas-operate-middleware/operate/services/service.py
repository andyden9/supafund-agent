    ...
    def _build_docker(
        ...
        for node in deployment["services"]:
            if "abci" in node:
                ...
                # ensure the agent persistent data directory is mounted
                has_data_mount = any(
                    m.split(":", 1)[0].endswith("persistent_data") or ":/data" in m
                ...
                if not has_data_mount:
                    ...
                    new_mappings.append("../persistent_data:/data:Z")

                log_dir_env = service.env_variables.get(AGENT_LOG_ENV_VAR, {}).get(
                    "value"
                )
                if log_dir_env:
                    log_mapping = f"../{AGENT_LOG_DIR}:{log_dir_env}:Z"
                    if log_mapping not in new_mappings:
                        new_mappings.append(log_mapping)
...
class Service(LocalResource):
    ...
    def consume_env_variables(self) -> None:
        ...
        # Ensure Docker deployments receive concrete RPC endpoints
        gnosis_rpc = self.env_variables.get("GNOSIS_LEDGER_RPC", {}).get("value")
        if gnosis_rpc:
            derived_rpc_map = {
                "GNOSIS_LEDGER_RPC_0": gnosis_rpc,
                "ETHEREUM_LEDGER_RPC_0": gnosis_rpc,
                "RPC_0": gnosis_rpc,
                "RPC_1": gnosis_rpc,
                "RPC_2": gnosis_rpc,
                "RPC_3": gnosis_rpc,
            ...
            updated = False
            for key, value in derived_rpc_map.items():
                os.environ[key] = str(value)
                if key not in self.env_variables:
                    self.env_variables[key] = {
                        "name": key,
                        "description": "Auto-derived RPC endpoint",
                        "value": str(value),
                        "provision_type": "computed",
                    }
                    updated = True
                else:
                    self.env_variables[key]["value"] = str(value)
                    updated = True

...
import { OLAS_CONTRACTS } from '@/supafund/core/config/olasContracts';
import {
...
import { ContractType } from '@/supafund/core/enums/Contract';
...
import { Maybe, Nullable } from '@/supafund/core/types/Util';
...
export abstract class StakedAgentService {
  ...
  static getCurrentStakingProgramByServiceId = async (
    ...
    try {
      ...
      const stakingProgramsOnChain = STAKING_PROGRAMS[evmChainId];
      if (!stakingProgramsOnChain) return null;

      // filter out staking programs that are not on the chain
      const stakingProgramEntries = Object.entries(stakingProgramsOnChain).filter(
        (entry) => {
          const [, program] = entry;

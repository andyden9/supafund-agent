...
import { PROVIDERS } from '@/supafund/core/constants/providers';
...
import {
  ...
  StakingRewardsInfo,
} from '@/supafund/core/types/Autonolas';
...
const getStakingProgramConfigOrThrow = (
  chainId: EvmChainId,
  stakingProgramId: StakingProgramId,
) => {
  const programsForChain = STAKING_PROGRAMS[chainId];
  if (!programsForChain) {
    throw new Error('Staking program not found');
  }
  const stakingProgramConfig = programsForChain[stakingProgramId];
  if (!stakingProgramConfig) {
    throw new Error('Staking program not found');
  }
  return stakingProgramConfig;
};
...
export abstract class SupafundService extends StakedAgentService {
  static getAgentStakingRewardsInfo = async ({
    ...
    const stakingProgramConfig = getStakingProgramConfigOrThrow(
      chainId,
      stakingProgramId,
    );
    const {
  ...
  static updateSupafundConfig = async (
    ...
    if (serviceConfigId) {
      const { ServicesService } = await import('@/supafund/core/services/Services');
  ...
  static getAvailableRewardsForEpoch = async (
    ...
    const stakingProgramConfig = getStakingProgramConfigOrThrow(chainId, stakingProgramId);
  ...
  static getServiceStakingDetails = async (
    ...
    const { contract: stakingTokenProxyContract } = getStakingProgramConfigOrThrow(chainId, stakingProgramId);
    const provider = PROVIDERS[chainId].multicallProvider;
    ...
    } as unknown as ServiceStakingDetails);
  };
  ...
  static getStakingContractDetails = async (
    ...
    const programsForChain = STAKING_PROGRAMS[chainId];
    if (!programsForChain) throw new Error('Staking program not found');
    const stakingProgramConfig = programsForChain[stakingProgramId];

...
import { AgentHealthCheckResponse } from '@/supafund/core/types/Agent';
import { ElectronStore, ElectronTrayIconStatus } from '@/supafund/core/types/ElectronApi';
import { browserStore, isBrowserEnvironment } from '@/supafund/core/utils/browserStore';
...
export const ElectronApiProvider = ({ children }: PropsWithChildren) => {
  const getElectronApiFunction = (functionNameInWindow: string) => {
    ...
    if (!hasElectronApi) {
      // Running in browser (not Electron), return undefined to use default context values
      return undefined;
    }

    const fn = get(window, `electronAPI.${functionNameInWindow}`);
    if (!fn || typeof fn !== 'function') {
      // Electron API exists but function not found, return undefined instead of throwing
      console.warn(`Function ${functionNameInWindow} not found in window.electronAPI`);
      return undefined;
    }
  ...
  // In browser mode, use localStorage-based store instead of Electron store
  const storeImplementation = isBrowserEnvironment()
    ? browserStore
    : {
  ...
  return (
    <ElectronApiContext.Provider
      value={{
        getAppVersion: getElectronApiFunction('getAppVersion') || (async () => 'Browser v0.1.0'),

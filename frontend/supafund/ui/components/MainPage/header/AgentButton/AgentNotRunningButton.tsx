...
import { MechType } from '@/supafund/core/config/mechs';
...
import { MasterEoa, MasterSafe } from '@/supafund/core/enums/Wallet';
...
import { MultisigOwners, useMultisigs } from '@/supafund/core/hooks/useMultisig';
...
import { updateServiceIfNeeded } from '@/supafund/core/utils/service';
...
const useServiceDeployment = () => {
  ...
  const deployAndStartService = useCallback(async () => {
    ...
    if (!service) {
      try {
        const mechType = selectedStakingProgramId
        ...
        middlewareServiceResponse = await ServicesService.createService({
          ...
          useMechMarketplace: mechType === MechType.Marketplace,
        });
      } catch (error) {

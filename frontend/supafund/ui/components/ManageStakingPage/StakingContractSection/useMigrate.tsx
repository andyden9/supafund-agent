...
export const useMigrate = (migrateToStakingProgramId: StakingProgramId) => {
  ...
  const minimumOlasRequiredToMigrate = useMemo(() => {
    ...
    return stakingProgramsForChain?.[migrateToStakingProgramId]
  ...
  const migrateValidation = useMemo<MigrateValidation>(() => {
    ...
    const stakingProgram = stakingProgramsForChain?.[migrateToStakingProgramId];
    ...
    const { deprecated: isDeprecated, agentsSupported } = stakingProgram;

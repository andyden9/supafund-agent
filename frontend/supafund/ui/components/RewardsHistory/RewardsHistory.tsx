...
import {
  ...
  ONE_DAY_IN_S,
} from '@/supafund/core/utils/time';

import {
  Checkpoint,
  useRewardsHistory,
} from '@/supafund/core/hooks/useRewardsHistory';
...
const ContractRewards = ({
  ...
  const stakingProgramsForChain =
    STAKING_PROGRAMS[selectedAgentConfig.evmHomeChainId];
  const stakingProgramMeta = stakingProgramsForChain?.[stakingProgramId];
  if (!stakingProgramMeta) {
    return null;
  }
...
export const RewardsHistory = () => {
  ...
  const history = useMemo(() => {
    ...
    const stakingProgramAddresses =
      STAKING_PROGRAM_ADDRESS[selectedAgentConfig.evmHomeChainId];

    if (!stakingProgramAddresses) return null;
